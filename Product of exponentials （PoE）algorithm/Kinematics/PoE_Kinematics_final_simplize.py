...
def MZ07_Trajectory_Planning_StraightLine_TriplePolynomial_Twist(θ1_origin, θ2_origin,θ3_origin,θ4_origin,θ5_origin,θ6_origin,θ1_target, θ2_target,θ3_target,θ4_target,θ5_target,θ6_target,T=3,t=0.01):
	...
	for i in np.arange(0,T+t,t):
		...
		# ω_Vst1List.append(ω_Vst1)
		# ω_Vst2List.append(ω_Vst2)
		# ω_Vst3List.append(ω_Vst3)
		# ω_VstList.append(np.sqrt(ω_Vst1*ω_Vst1 + ω_Vst2*ω_Vst2 + ω_Vst3*ω_Vst3))

		ωθ_Vst1.append(ω_Vst1*θ_Vst)
		ωθ_Vst2.append(ω_Vst2*θ_Vst)
		ωθ_Vst3.append(ω_Vst3*θ_Vst)
		ωθ_Vst.append(np.sqrt(ωθ_Vst1[-1]*ωθ_Vst1[-1] + ωθ_Vst2[-1]*ωθ_Vst2[-1] + ωθ_Vst3[-1]*ωθ_Vst3[-1]))

	x_axis = [i for i in range(len(ωθ_st1))]
	plt.figure()
	#plta = plt.gca(projection='3d')
	#plta.plot(px, py, pz)
	# plt.plot(x_axis,ωθ_Vst1)
	# plt.plot(x_axis,ωθ_Vst2)
	# plt.plot(x_axis,ωθ_Vst3)
	plt.plot(x_axis,ωθ_Vst)

	# plt.plot(x_axis,ω_Vst1List)
	# plt.plot(x_axis,ω_Vst2List)
	# plt.plot(x_axis,ω_Vst3List)
	#plt.plot(x_axis,ω_VstList)
...
MZ07_Trajectory_Planning_StraightLine_TrapezoidalMotion_Parameter_v_T(10,10,10,10,10,10,11,11,11,11,11,11,v = 10, T=0.15,t=0.01)
